...
static InterHdlr_fptr sHandlers[INTERRUPT_HANDLER_NUM];	// This is an Array saving all of interrupt handler function
...
void Hal_interrupt_enable(uint32_t interrupt_num)
{
	...
	else
	{
		...
		SET_BIT(GicDist->setenable2, bit_num); // ((GicDist->setenable2) |= (1 << bit_num))
	}
}
...
void Hal_interrupt_disable(uint32_t interrupt_num)
{
	...
	if(bit_num < GIC_IRQ_START)
	{
		CLR_BIT(GicDist->setenable1, bit_num); // ((GicDist->setenable1) &= ~(1 << bit_num))
	}
	else
	{
		bit_num -= GIC_IRQ_START;
...
void Hal_interrupt_register(InterHdlr_fptr handler, uint32_t interrupt_num)
{
